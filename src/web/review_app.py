...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    .vt-head_pose { background: rgba(59,130,246,0.2); color: #60a5fa; }
    .vt-gaze { background: rgba(56,189,248,0.2); color: #38bdf8; }
    .vt-no_face { background: rgba(239,68,68,0.2); color: #f87171; }
    .vt-multiple_faces { background: rgba(245,158,11,0.2); color: #fbbf24; }
    .vt-phone_detected { background: rgba(45,212,191,0.2); color: #2dd4bf; }
    .vt-multiple_people { background: rgba(249,115,22,0.2); color: #fb923c; }
    ...
    for i, subject in enumerate(valid_subjects):
        with cols[i % 3]:
            ...
            for vt, count in sorted(vtypes.items(), key=lambda x: x[1], reverse=True)[:3]:
                vtype_html += f'<span class="violation-type-badge vt-{vt}">{vt}: {count}</span> '
...
def render_review(subject_id: str):
    ...
    with video_col:
        # Toggle overlays
        show_overlays = st.toggle("Show Overlays (Facemesh, Objects, Detections)", value=False)
    ...
    <style>
        ...
        .violation-type-badge { padding: 2px 8px; border-radius: 6px; font-size: 0.7rem; font-weight: 600; white-space: nowrap; }
        .vt-head_pose { background: rgba(59,130,246,0.2); color: #60a5fa; }
        .vt-gaze { background: rgba(56,189,248,0.2); color: #38bdf8; }
        .vt-no_face { background: rgba(239,68,68,0.2); color: #f87171; }
        .vt-multiple_faces { background: rgba(245,158,11,0.2); color: #fbbf24; }
    ...
    if duration > 0:
        ...
        for v in violations:
            ...
            colors = {
                ...
                "phone_detected": "#2dd4bf",
                "multiple_people": "#f97316",
            }
            color = colors.get(vtype, "#3b82f6")
            tooltip = f"{vtype}: {format_time(v['start_time'])} → {format_time(v['end_time'])} ({v.get('duration', 0):.1f}s) — {msg}"
        ...
        <div class="timeline-container">
            <div class="legend">
                <div class="legend-item"><span class="legend-dot" style="background:#3b82f6;"></span> Head Pose</div>
                <div class="legend-item"><span class="legend-dot" style="background:#38bdf8;"></span> Gaze</div>
                <div class="legend-item"><span class="legend-dot" style="background:#ef4444;"></span> No Face</div>
                <div class="legend-item"><span class="legend-dot" style="background:#f59e0b;"></span> Multiple Faces</div>
                <div class="legend-item"><span class="legend-dot" style="background:#2dd4bf;"></span> Phone</div>
                <div class="legend-item"><span class="legend-dot" style="background:#f97316;"></span> Multi People</div>
            </div>
    ...
    if violations:
        for v in violations:
            ...
            <div class="violation-row" onclick="seekVideo({v['start_time']})">
                <span class="violation-type-badge vt-{vtype}">{vtype}</span>
        ...
        if vtypes:
            for vtype, count in sorted(vtypes.items(), key=lambda x: x[1], reverse=True):
                ...
                bar_color = {
                    ...
                    "multiple": "#f59e0b",
                    "phone": "#2dd4bf",
                }.get(vtype.split("_")[0] if "_" in vtype else vtype, "#3b82f6")
...
def main():
    ...
    if st.session_state.page == "review" and st.session_state.selected_subject:
        ...
        if index_data:
            ...
            st.markdown("""
            1. **Run the batch processor** to analyze all videos:
            ```bash
